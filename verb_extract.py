...
flags.DEFINE_string("task_name", 'datas', "The name of the task to train.")
...
flags.DEFINE_integer("cnn_size", 50, "Total batch size for predict.")
...
class DatasProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if line == [] or i == len(lines)-1:
          ...
          continue
      if set_type == "test":
        text_a += tokenization.convert_to_unicode(line[0]).split(' ')[0]
        label.append(tokenization.convert_to_unicode(line[0]).split(' ')[1])
      else:
        text_a += tokenization.convert_to_unicode(line[0]).split(' ')[0]
        label.append(tokenization.convert_to_unicode(line[0]).split(' ')[1])
    # print(max_len)
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if isinstance(example, PaddingInputExample):
    return InputFeatures(
        ...
        wubi_inputs=[0] * 4 * max_seq_length,
  ...
  if not os.path.exists(FLAGS.CNNpic_dir + 'data_dict.npy'):
      data_process.save_cnn_inputs(FLAGS.CNNpic_dir)
  cnn_inputs = data_process.get_cnn_inputs(tokens, max_seq_length, np.load(FLAGS.CNNpic_dir + 'data_dict.npy').item())
  wubi_inputs = data_process.wubi_input(tokens, max_seq_length)
  '''修改'''
  label_ids = []
  for example_label in example.label[:max_seq_length]:
      label_ids.append(label_map[example_label])
  while len(label_ids) < max_seq_length:
      label_ids.append(label_map['[SPC]'])
...
def file_based_convert_examples_to_features(
  ...
  def create_preload():
      ...
      for (ex_index, example) in enumerate(examples):
        ...
        features["wubi_inputs"] = create_int_feature(feature.wubi_inputs)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "wubi_inputs": tf.FixedLenFeature([seq_length * 4], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  blstm_crf = BLSTM_CRF(embedded_chars=embedding, hidden_unit=lstm_size, cell_type=cell, num_layers=num_layers,
                        ...
                        cnn_input=cnn_input, wubi_input=wubi_input,
                        is_cnn=True, is_gru=True)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    wubi_input = features['wubi_inputs']
    ...
    '''修改'''
    total_loss, logits, trans, pred_ids, one_weight = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
        num_labels, False, cnn_input, wubi_input,
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.train.LoggingTensorHook(hook, every_n_iter=1)
...
def main(_):
  ...
  if FLAGS.do_train:
    train_file = "output/train.tf_record"
    #将数据存储在train_file里
  ...
  if FLAGS.do_eval:
    ...
    eval_file = "output/eval.tf_record"
    file_based_convert_examples_to_features(
    ...
    eval_input_fn = file_based_input_fn_builder(
        input_file='output/eval.tf_record',
        seq_length=FLAGS.max_seq_length,
        is_training=False,
  ...
  if FLAGS.do_predict:
    ...
    predict_file = 'output/eval.tf_record'
...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        flags.DEFINE_bool("delete_preload", False, "et.")
        flags.DEFINE_bool(
            "do_predict", True,
            "Whether to run the model in inference mode on the test set.")
        ...
        flags.DEFINE_float("learning_rate", 0.0002, "The initial learning rate for Adam.")
        ...
        flags.DEFINE_string(
            ...
            "The output directory where the model checkpoints will be written.")
        if os.path.exists(FLAGS.output_dir + '/test_results.tsv'):
            os.remove(FLAGS.output_dir + '/test_results.tsv')
        flags.DEFINE_string(
            "CNNpic_dir", '小篆/',
            "CNNpic_dir")
